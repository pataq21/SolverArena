...
def build_model(num_groups, num_tables, group_sizes, table_capacities, people_groups):
    ...
    model.People = Set(
        initialize=[p for group in people_groups.values() for p in group])

    # Parameters
    model.group_size = Param(
        model.Groups, initialize=lambda model, g: group_sizes[g - 1])
    model.table_capacity = Param(
        model.Tables, initialize=lambda model, t: table_capacities[t - 1])
    model.person_to_group = Param(
        model.People, initialize=lambda model, p: next(
            g for g, people in people_groups.items() if p in people)
    )
    ...
    print("Variable 3")
    model.group_present = Var(
        model.Groups, model.Tables, domain=Binary)  # New binary variable

    # Objective Function
    model.objective = Objective(
        expr=sum(model.group_split[g] for g in model.Groups), sense=minimize)
    ...
    model.person_assignment_constraint = Constraint(
        model.People, rule=person_assignment_constraint)
    ...
    model.table_capacity_constraint = Constraint(
        model.Tables, rule=table_capacity_constraint)
    ...
    def group_assignment_constraint(model, g):
        return (
            sum(model.assignment[p, t]
                for p in model.People if model.person_to_group[p] == g for t in model.Tables)
            == model.group_size[g]
        )

    model.group_assignment_constraint = Constraint(
        model.Groups, rule=group_assignment_constraint)
    ...
    def group_presence_constraint(model, g, t):
        return (
            sum(model.assignment[p, t]
                for p in model.People if model.person_to_group[p] == g)
            <= model.table_capacity[t] * model.group_present[g, t]
        )

    model.group_presence_constraint = Constraint(
        model.Groups, model.Tables, rule=group_presence_constraint)
    ...
    model.group_split_constraint = Constraint(
        model.Groups, rule=group_split_constraint)

    return model


def main(dataset_name):
    num_groups, num_tables, group_sizes, table_capacities, people_groups = create_data(
        dataset_name)
    model = build_model(num_groups, num_tables, group_sizes,
                        table_capacities, people_groups)
    model.write(f"mps_files/model_{dataset_name}.mps",
